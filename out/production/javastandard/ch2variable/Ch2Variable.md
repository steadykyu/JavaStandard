...
> 음수의 2진표현 구하기
+ 음수의 절대값을 2진법으로 바꾼다.
+ 1 의보수를 구하고 + 1 을하면 2의 보수를 구할 수 있다.
+ 그럼 음수 완성
...
### 4.2 char
+ 변수 선언시 문자가 저장되는 것이 아닌 문자의 유니코드(정수)가 저장된다.
...
> 정수형의 선택기준
+ JVM 피연산자가 4byte로 저장하므로 int를 쓰는게 좋다.
+ +-20억을 넘어가면 long 자료형을 사용하자.
+ 오버플로우 될시 최대값 +1 = 최소값 / 최소값 -1 = 최대값이 된다.

### 4.4 실수형
+ 실수형에서는 오버플로우가 되면 변수가의 값은 무한대가 된다.
...
### 5.6 자동 형변환
+ 기존의 값을 최대한 보존할수 있는 타입으로 자동 형변환 한다.
